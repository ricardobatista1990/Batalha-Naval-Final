...
public class Tabuleiro implements Serializable {
    ...
    /**
     * Serializa apenas a grelha em 100 caracteres (sem newlines) para uso no protocolo de rede.
     * Usado na retoma de jogo guardado.
     */
    public String serializarGrelha() {
        StringBuilder sb = new StringBuilder(100);
        for (int i = 0; i < TAMANHO; i++)
            for (int j = 0; j < TAMANHO; j++)
                sb.append(grelha[i][j]);
        return sb.toString();
    }

    /**
     * Desserializa uma grelha compacta (100 chars) recebida via protocolo.
     */
    public static char[][] desserializarGrelha(String dados) {
        char[][] g = new char[TAMANHO][TAMANHO];
        for (int i = 0; i < TAMANHO; i++)
            for (int j = 0; j < TAMANHO; j++)
                g[i][j] = dados.charAt(i * TAMANHO + j);
        return g;
    }
