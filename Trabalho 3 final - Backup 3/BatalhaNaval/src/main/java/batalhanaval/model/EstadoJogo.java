...
public class EstadoJogo implements Serializable {
    ...
    /**
     * Pasta de saves: junto ao JAR/executável, ou junto ao diretório de trabalho.
     * Assim funciona tanto no IntelliJ como ao correr o JAR por duplo clique.
     */
    private static final String PASTA_SAVES = resolverPastaSaves();

    private static String resolverPastaSaves() {
        try {
            // Tenta colocar os saves junto ao JAR
            java.net.URL url = EstadoJogo.class.getProtectionDomain()
                    .getCodeSource().getLocation();
            java.io.File jar = new java.io.File(url.toURI());
            java.io.File pasta = jar.isFile()
                    ? new java.io.File(jar.getParentFile(), "saves")
                    : new java.io.File("saves");
            return pasta.getAbsolutePath() + java.io.File.separator;
        } catch (Exception e) {
            return "saves" + java.io.File.separator;
        }
    }
