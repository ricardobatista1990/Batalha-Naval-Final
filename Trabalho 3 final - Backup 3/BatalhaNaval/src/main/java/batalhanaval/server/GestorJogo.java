...
public class GestorJogo {
    ...
    private boolean desconexaoTratada = false;

    public GestorJogo(EstadoJogo estado, ServerSocket serverSocket) {
        this.estado = estado;
        this.serverSocket = serverSocket;

        // Shutdown hook: guarda o jogo sempre que a JVM terminar
        // (cobre System.exit, fechar janela, Ctrl+C, crash, etc.)
        Runtime.getRuntime().addShutdownHook(new Thread(() -> {
            if (estado.getFase() == FaseJogo.EM_JOGO
                    || estado.getFase() == FaseJogo.COLOCAR_BARCOS) {
                try {
                    String f = estado.guardar();
                    System.out.println("[Servidor] Jogo guardado automaticamente no shutdown: " + f);
                } catch (IOException e) {
                    System.out.println("[Servidor] Erro ao guardar no shutdown: " + e.getMessage());
                }
            }
        }, "ShutdownSave"));
    }
    ...
    public void iniciar() throws IOException {
        ...
        // Restaurar nomes dos handlers se jogo carregado
        if (estado.getNomeJogador1() != null) {
            handler1.setNome(estado.getNomeJogador1());
        }
        if (estado.getNomeJogador2() != null) {
        ...
        // Verificar se é um jogo retomado (já estava em curso)
        boolean jogoRetomado = estado.getFase() == FaseJogo.EM_JOGO
                || (estado.getFase() == FaseJogo.COLOCAR_BARCOS && estado.todosBarcosColocados());

        if (jogoRetomado) {
            // Jogo retomado: enviar JOGO_RETOMADO com estado completo
            System.out.println("A retomar jogo guardado...");
            estado.setFase(FaseJogo.EM_JOGO);

            handler1.enviar(Protocolo.montar(Protocolo.JOGO_RETOMADO, nome2,
                    estado.getTabuleiro(1).serializarGrelha(),
                    estado.getTabuleiro(2).serializarGrelha(),
                    String.valueOf(estado.getJogadorAtual())));
            handler2.enviar(Protocolo.montar(Protocolo.JOGO_RETOMADO, nome1,
                    estado.getTabuleiro(2).serializarGrelha(),
                    estado.getTabuleiro(1).serializarGrelha(),
                    String.valueOf(estado.getJogadorAtual())));
            ...
            // Novo jogo: fase de colocação de barcos
    ...
    public synchronized void tratarDesconexao(int numJogador) {
        ...
        if (desconexaoTratada) return;
        desconexaoTratada = true;

        System.out.println("[Servidor] Jogador " + numJogador + " desconectou-se.");

        // Guardar estado automaticamente se o jogo estava em curso
