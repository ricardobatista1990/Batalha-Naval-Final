...
public class Cliente {
    ...
    private void processar(String mensagem) {
        ...
        switch (comando) {
            ...
            case Protocolo.JOGO_RETOMADO -> {
                // JOGO_RETOMADO|nomeAdversario|meuGrelha(100)|grelhaAdv(100)|jogadorAtual
                nomeAdversario = partes.length > 1 ? partes[1] : "Adversário";
                if (partes.length > 3) {
                    char[][] meuG = Tabuleiro.desserializarGrelha(partes[2]);
                    char[][] advG = Tabuleiro.desserializarGrelha(partes[3]);
                    for (int i = 0; i < 10; i++) {
                        meuTabuleiro.getGrelha()[i] = meuG[i].clone();
                        tabuleirOAdversario.getGrelha()[i] = advG[i].clone();
                    }
                }
                System.out.println("\n✅ Jogo retomado! Adversário: " + nomeAdversario);
                mostrarTabuleiros();
            }
