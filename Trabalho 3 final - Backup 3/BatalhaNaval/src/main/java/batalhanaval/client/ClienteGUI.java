...
public class ClienteGUI extends JFrame {
    ...
    private JButton btnLigar;   // referência para reabilitar após voltarAJogar
    private JLabel  errLigar;
    ...
    private JPanel panelConectar() {
        ...
        btnLigar = btn; // guardar referência para reabilitar depois
        g.gridy = 6; g.gridx = 0; g.gridwidth = 2; g.insets = new Insets(14, 30, 6, 30);
        p.add(btn, g);

        JLabel err = label("", Font.PLAIN, 12);
        errLigar = err; // guardar referência para limpar depois
    ...
    private JPanel panelAguardar() {
        JPanel p = bgPanel(new GridBagLayout());
        p.setPreferredSize(new Dimension(460, 240));
        ...
        JButton btnCancelar = btn("✕  Cancelar", new Color(160, 35, 35));
        btnCancelar.setFont(new Font("SansSerif", Font.BOLD, 13));
        btnCancelar.setBorder(new EmptyBorder(8, 24, 8, 24));
        btnCancelar.addActionListener(e -> voltarAJogar());
        p.add(btnCancelar, gc);
    ...
    private JPanel panelJogo() {
        ...
        JButton btnGuardar = btn("💾  Guardar", new Color(45, 95, 160));
        btnGuardar.setFont(new Font("SansSerif", Font.PLAIN, 12));
        btnGuardar.setBorder(new EmptyBorder(5, 10, 5, 10));
        btnGuardar.setToolTipText("Guardar o estado do jogo");
        btnGuardar.addActionListener(e -> {
            if (emJogo && netOut != null) {
                netOut.println(Protocolo.GUARDAR_JOGO);
            }
        });
        eastBar.add(btnGuardar);
    ...
    private void ligar(String host, int porta, JButton btn, JLabel err) {
        try {
            ...
            if (emJogo) {
                SwingUtilities.invokeLater(() -> {
                    // Ligação perdida inesperadamente — voltar ao ecrã de ligação
                    emJogo = false;
                    if (btnLigar != null) btnLigar.setEnabled(true);
                    if (errLigar != null) {
                        errLigar.setText("Ligação perdida. Tenta novamente.");
                        errLigar.setForeground(DANGER);
                    }
                    aguardaResposta = false;
                    meuTurno = false;
                    for (char[] row : meuGrid) java.util.Arrays.fill(row, ' ');
                    for (char[] row : advGrid)  java.util.Arrays.fill(row, ' ');
                    cards.show(root, "CONECTAR");
                    repack();
                });
            }
    ...
    private void processar(String msg) {
        ...
        switch (p[0]) {
            ...
            case Protocolo.JOGO_RETOMADO     -> onJogoRetomado(p);
    ...
    private void onJogoRetomado(String[] p) {
        // JOGO_RETOMADO|nomeAdversario|meuGrelha(100)|grelhaAdv(100)|jogadorAtual
        nomeAdv = p.length > 1 ? p[1] : "Adversário";
        if (p.length > 3) {
            char[][] meuG  = Tabuleiro.desserializarGrelha(p[2]);
            char[][] advG  = Tabuleiro.desserializarGrelha(p[3]);
            for (int i = 0; i < 10; i++) {
                System.arraycopy(meuG[i],  0, meuGrid[i],  0, 10);
                for (int j = 0; j < 10; j++) {
                    // Nunca colocar 'N' no tabuleiro do adversário (seria trapaça)
                    // Converter 'N' em ' ' — só manter resultados de tiros
                    char c2 = advG[i][j];
                    advGrid[i][j] = (c2 == 'X' || c2 == 'O' || c2 == 'A') ? c2 : ' ';
                }
            }
        }
        log("✅ Jogo retomado! Adversário: " + nomeAdv);
        turnoLbl.setText("Jogo retomado – a aguardar turno...");
        turnoLbl.setForeground(TEXT);
        cards.show(root, "JOGO");
        repack();
        if (tMeu != null) tMeu.repaint();
        if (tAdv != null) tAdv.repaint();
    }
    ...
    private void onResultadoTiro(String[] p) {
        ...
        // Resposta recebida — reabilitar cliques se ainda é o nosso turno
        aguardaResposta = false;
        if (meuTurno && tirosRestantes > 0) tAdv.setClicavel(true);
    }
    ...
    private void voltarAJogar() {
        ...
        // Reabilitar botão de ligação para nova tentativa
        if (btnLigar != null)  { btnLigar.setEnabled(true); }
        if (errLigar != null)  { errLigar.setText(""); }
        aguardaResposta = false;
        cards.show(root, "CONECTAR");
        repack();
    }
    ...
    // Flag que bloqueia cliques duplos enquanto aguarda resposta do servidor
    private volatile boolean aguardaResposta = false;

    private void atirar(int l, int c) {
        if (!meuTurno || tirosRestantes <= 0) return;
        // Verificar apenas resultados de tiros (X=acerto, O=água, A=afundado)
        // NÃO verificar 'N' porque navios do adversário não devem ser visíveis no advGrid
        char celula = advGrid[l][c];
        if (celula == 'X' || celula == 'O' || celula == 'A') {
            log("⚠  Já disparaste nessa posição (" + coord(l, c) + ").");
            return;
        }
        if (aguardaResposta) return; // evita duplo clique durante latência de rede
        aguardaResposta = true;
        tAdv.setClicavel(false); // desabilitar temporariamente até chegar resposta
